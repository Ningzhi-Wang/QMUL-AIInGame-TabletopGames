...
class BasicTreeNode {
    ...
    private BasicTreeNode treePolicy() {
        ...
        while (cur.state.isNotTerminal() && cur.depth < player.params.maxTreeDepth) {
            if (!cur.unexpandedActions().isEmpty()) {
                ...
                AbstractAction actionChosen = cur.thompson();
    ...
    private AbstractAction thompson() {
        ...
        for (AbstractAction action : children.keySet()) {
            ...
            if (child == null)
                ...
                bestAction = action;
            double sampleValue = rnd.nextGaussian() / Math.sqrt(tau) + mean;
            boolean iAmMoving = state.getCurrentPlayer() == player.getPlayerID();
            double thompsonValue = iAmMoving ? sampleValue : -sampleValue;
            if (thompsonValue > bestValue) {
                bestAction = action;
                bestValue = thompsonValue;
            }

        }

        if (bestAction == null)
            throw new AssertionError("We have a null value in Thompson: shouldn't really happen!");
    ...
    private double rollOut() {
        ...
        if (player.params.rolloutLength > 0) {
            while (!finishRollout(rolloutState, rolloutDepth)) {
                AbstractAction next = randomPlayer.getAction(rolloutState, randomPlayer.getForwardModel()
    ...
    private void backUp(double result) {
        ...
        while (n != null) {
            n.nVisits++;
            n.totValue += result;
            n.mean = (n.tau * n.mean + n.totValue) / (n.tau + n.nVisits);
            n.tau = n.tau + 1;
            n = n.parent;
        }
    }
